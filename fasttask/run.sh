...
# 检查环境变量node_type是否已设置
    ...
    nohup cd files && redis-server --bind "$master_host" --requirepass "$task_queue_passwd" --port "$task_queue_port" --save $rdb_snapshot_gap 1 >/dev/null 2>&1 &
    ...
    export task_queue_port=6379
    nohup redis-server --bind "$master_host" --requirepass "$task_queue_passwd" --port "$task_queue_port" --save $rdb_snapshot_gap 1 >/dev/null 2>&1 &
    nohup uvicorn api:app --host 0.0.0.0 --port 80 --reload >/var/log/uvicorn.log 2>&1 &
    tail -f /var/log/redis/*.log /var/log/celery.log /var/log/uvicorn.log /var/log/celery/*.log
